...
app.get("/", (req, res) => {
  ...
  // 수정 후 자동으로 리빌드 되는건 자연스러운게 아니다
  // 무언가 툴들이 도는 것.. 여기선 nodemon
});
...
app.post("/", async (req, res) => {
  ...
  console.log(`forImageEnglishText : ${prompt}`);
  ...
  console.log(`toJsonString : ${promptJSON}`);
  ...
  console.log(`createDescription : ${prompt2}`);
  // 3-2. 그거에서 프롬프트만 추출
  // mixtral-8x7b-32768 (groq)
  const promptJSON2 = await callAI({
    ...
    model: MIXTRAL_MODEL,
    // text,
    text: `${prompt2}에서 reasoning을 위해 작성된 200자 이내의 한글 프롬프트를 JSON Object로 prompt라는 key로 JSON string으로 output해줘, 한글로`,
    jsonMode: true,
  }).then((res) => JSON.parse(res.choices[0].message.content).prompt);
  console.log(`toJsonString : ${promptJSON2}`);
